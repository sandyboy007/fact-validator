...
\chapter*{Abstract}
This thesis presents Fact Validator, a deployment-oriented system for fake-news and misinformation analysis that estimates the likelihood that content is false or misleading. The architecture accepts URLs or raw text, extracts fact-like claims, retrieves supporting and contradicting evidence, scores source credibility with an auditable rubric, and aggregates claim-level verdicts into a final misinformation likelihood score. The thesis positions the project against two relevant research directions: Zero-shot Faithful Factual Error Correction and FacTool. The first is a faithful correction framework; the second is a tool-augmented factuality framework. Fact Validator differs from both by combining transparent source scoring, operational persistence, versioned benchmarking, and explicit cost analysis in a full-stack application. On the strongest frozen benchmark family in the repository, a 5000-claim mixed evaluation set built from FEVER, LIAR, SciFact, and a normalized health-domain corpus, the default full pipeline reaches 50.82\% accuracy, the tuned best deployment variant reaches 50.98\%, and the highest-scoring frozen ablation reaches 51.34\%. The project therefore supports a cautious but positive thesis claim: its main novelty is not universal superiority in factuality classification, but a stronger integration of transparency, architecture, and deployment realism.
...
Misinformation detection is not only a classification problem. A realistic system must decide what to verify, where to search for evidence, how much to trust each source, how to explain a verdict, and how to stay usable when external tools fail. Most benchmark-centered papers optimize one slice of this workflow. This thesis instead studies the complete pipeline required for practical fact checking.

The goal of Fact Validator is to assess the likelihood that content is fake or misleading by combining retrieval, credibility reasoning, claim-level verification, and a user-facing analysis interface. The motivating research question is the following:
...
\begin{enumerate}
  ...
  \item measure both predictive and operational performance,
...
The evaluation includes the default full pipeline, tuned and ablated variants, and simple baselines. This allows the thesis to separate three questions:

\begin{enumerate}
  \item Does the full system beat naive comparators?
...
\begin{enumerate}
  ...
  \item confidence and calibration error,
...
\begin{tabular}{lrrrr}
\toprule
System & Accuracy & 95\% CI & Avg. Confidence & Calibration Error \\
\midrule
...
The results support a careful but positive interpretation. The default full pipeline beats the majority baseline by 1.56 percentage points and the length heuristic by 1.40 percentage points on the strict-validation snapshot. The best tuned deployment-oriented variant reaches 50.98\% accuracy. The single highest frozen result is the no-debate ablation at 51.34\%, which shows that the debate layer should be used selectively rather than universally.

\section{Per-Dataset Behavior}

\begin{table}[H]
\centering
\caption{Per-dataset default full-pipeline performance.}
...
This table explains where the gains come from. The architecture is strongest on health and scientific claims and weakest on FEVER. This pattern supports the thesis that explicit source-sensitive verification is particularly useful when domains vary in reliability and evidence type.
...
Metric & Value \\
\midrule
Baseline latency & 8.20 s/claim \\
Debate latency & 72.00 s/claim \\
Debate overhead & 8.78x \\
Baseline throughput & 439.02 claims/hour \\
Debate throughput & 50.00 claims/hour \\
Monthly cost without caching & USD 77.00 \\
Monthly cost with caching & USD 22.00 \\
Monthly savings & 71.43\% \\
\bottomrule
\end{tabular}
\end{table}

These numbers are part of the research value. Many fact-checking papers ignore system cost. This thesis does not.
...
\begin{enumerate}
  \item it is better as a deployment-ready misinformation analysis system,
  \item it is better in transparency of source trust assignment,
  \item it is better documented in terms of operational cost and reproducibility,
  \item it is more complete as an end-to-end research artifact.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item optional debate arbitration inside a broader deterministic pipeline,
...
\begin{enumerate}
  \item the full pipeline remains overconfident relative to its observed accuracy,
...
Fact Validator should be defended as a transparent and deployment-ready misinformation analysis architecture. Its strongest frozen benchmark family shows a modest but statistically significant edge over simple baselines, while its larger scientific value lies in explicit source-trust modeling, persistent evidence workflows, and cost-aware large-scale evaluation. Compared with Zero-shot Faithful Factual Error Correction and FacTool, the system is most novel not because it dominates their native tasks, but because it turns factuality research into a practical, auditable, end-to-end platform.
