...
\title{Fact Validator: A Transparent Fact-Checking Architecture with Auditable Credibility Scoring and a Versioned 5000-Claim Proxy Evaluation}
...
\begin{abstract}
Fact Validator is an end-to-end fact-checking architecture for misinformation risk assessment over free text and URL-derived content. The live application decomposes input into claims, retrieves web evidence, applies an auditable domain-credibility prior, semantically reranks evidence, and aggregates claim-level verdicts into a final misinformation likelihood score. The repository also contains a separate deterministic FactValidator-Proxy evaluated on a frozen 5000-claim benchmark assembled from FEVER, LIAR, SciFact, and the PUBHEALTH \texttt{health\_fact} dataset. The proxy combines lexical classification, category-level priors, heuristic semantic signals, deterministic arbitration, and a quality filter; it does not execute SerpAPI, live domain scoring, SentenceTransformer reranking, or Ollama debate for every test claim. The full proxy reaches 50.82\% accuracy and 0.4384 macro-F1, while the no-debate proxy reaches the best observed result of 51.34\% and 0.4427. The contribution is a deployment-ready, auditable research artifact and transparent proxy evaluation rather than a claim of state-of-the-art predictive performance.
\end{abstract}
...
This paper makes three bounded claims. First, Fact Validator provides a reproducible architecture for source-aware claim verification and misinformation likelihood scoring. Second, the repository contains a versioned 5000-claim deterministic proxy evaluation with frozen predictions and paired statistical artifacts. Third, the strongest contribution remains architectural and methodological rather than a claim of state-of-the-art task performance.

\subsection{Evaluation Boundary}
The live application and benchmark proxy are related but distinct artifacts. The live application implements SerpAPI retrieval, auditable domain-level credibility scoring, SentenceTransformer reranking, persistence, caching, and optional Ollama debate. The 5000-claim experiment evaluates \texttt{full\_proxy}; its components are deterministic approximations suitable for frozen, repeatable comparison. No live-application component is claimed to have executed for every benchmark item.
...
Caching is treated as a first-class architectural variable rather than an implementation detail. The live system memoizes normalized claims with a 24-hour TTL and stores previous analyses. The reported 71.43\% cost reduction is a scenario projection based on declared assumptions, not a controlled concurrent load-test measurement.

\section{Evaluation Protocol}

\subsection{Benchmark Family}

The strongest frozen evaluation surface in the repository is the versioned 5000-claim proxy benchmark recorded in the 2026-07-02 result family. The benchmark is assembled from FEVER \cite{thorne2018fever}, LIAR \cite{wang2017liar}, SciFact \cite{wadden2020fact}, and the PUBHEALTH \texttt{health\_fact} dataset stored under a legacy healthver compatibility name. The test split contains 5000 claims, with 11986 training claims and 2997 validation claims.

More precisely, the legacy healthver compatibility slot was populated with the PUBHEALTH \texttt{health\_fact} dataset rather than the original HealthVer release. The import pipeline sampled 8000 FEVER v1.0 claims, 6000 LIAR claims, 1200 SciFact claims, and 6000 \texttt{health\_fact} claims, then deduplicated normalized claim text to retain 19983 unique claims before splitting. Exact textual duplicates were excluded across train, validation, and test partitions by withholding all normalized test claims from the remaining pool.
...
Legacy healthver compatibility slot & PUBHEALTH health\_fact & 6000 & 1490 & 2 $\rightarrow$ SUPPORTED; 0 $\rightarrow$ REFUTED; 1 and 3 $\rightarrow$ NEI \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\begin{itemize}
  \item \textbf{Full FactValidator-Proxy} (artifact name: \texttt{full\_proxy}),
  \item \textbf{FEVER-aware exploratory proxy} (artifact name: \texttt{tune\_fever}),
  \item \textbf{Proxy without deterministic debate} (artifact name: \texttt{ablate\_debate}),
...
We report accuracy, macro-F1, Wilson accuracy intervals, a raw-score confidence diagnostic, per-dataset results, and operational scenario projections for latency, throughput, and cost. Exact two-sided McNemar tests, paired-bootstrap intervals with seed 42, Holm-adjusted p-values, paired risk differences, and matched-pair odds ratios are computed from aligned claim-level predictions. Because the proxy stores only one heuristic score rather than three class probabilities, no probability calibration or multiclass Brier score is claimed.
...
\toprule
System & Accuracy & Macro-F1 & 95\% Wilson CI & Avg. raw score & Raw-score gap \\
\midrule
Proxy without deterministic debate & 0.5134 & 0.4427 & [0.4995, 0.5273] & 88.37 & 0.3703 \\
FEVER-aware exploratory proxy & 0.5098 & 0.4421 & [0.4959, 0.5237] & 88.53 & 0.3755 \\
Full FactValidator-Proxy & 0.5082 & 0.4384 & [0.4943, 0.5221] & 88.31 & 0.3749 \\
...
The full proxy exceeds the majority baseline by 1.56 percentage points and the length heuristic by 1.40 percentage points. Exact two-sided McNemar tests give unadjusted p-values of 0.0433 and 0.0462 respectively; these marginal comparisons are not described as robust superiority after Holm correction. The no-debate proxy reaches 51.34\%, 0.52 points above the full proxy, with an unadjusted exact paired p-value of 0.00955. This result concerns deterministic proxy arbitration and does not establish the effect of live Ollama debate.

\begin{table}[htbp]
\centering
\caption{Per-dataset comparison for the full deterministic proxy.}
\label{tab:dataset}
\footnotesize
...
The per-dataset view matters more than the aggregate alone. The proxy's largest descriptive margins occur on SciFact and health-domain claims; it is positive on LIAR and slightly negative on FEVER. This pattern demonstrates distribution-specific behavior, not the effectiveness of live retrieval or source scoring, because those components were not executed for every proxy item.
...
\begin{enumerate}
  ...
  \item \textbf{Operational projections alongside evaluation.} The paper reports clearly separated cost and latency scenarios alongside proxy prediction quality, which is often omitted in fact-checking papers.
...
The strongest statistical limitation is the proxy's raw confidence diagnostic. Its average heuristic score is much higher than observed accuracy, but the score is not a multiclass probability and no proper Brier score is claimed. The proxy result therefore supports decision-support use, not unchecked automated adjudication. Deterministic proxy debate is another limitation: it adds complexity without helping the frozen benchmark. The effect of live Ollama debate remains a separate empirical question.

\section{Conclusion}

Fact Validator is best positioned as a transparent systems contribution for practical misinformation analysis. On the frozen 5000-claim benchmark, the deterministic proxy delivers modest raw differences over simple baselines, but the majority and length comparisons do not survive Holm correction. The broader contribution is architectural: auditable source-trust modeling, persistent evidence workflows, a reproducible proxy evaluation, and cost-aware deployment scenarios. Relative to Zero-shot Faithful Factual Error Correction and FacTool, the project is most novel in how it operationalizes factuality research into a deployable and inspectable verification stack. Future work should store class-probability vectors, freeze a licensed evidence corpus, evaluate the live system on an external benchmark, and measure operational behavior through repeated controlled load tests.
