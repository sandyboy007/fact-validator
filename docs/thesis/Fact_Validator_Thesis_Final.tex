...
The thesis-support apparatus used for this revision is on the dedicated thesis
correction branch; it is not represented as being on \texttt{main}. Before
submission, the exact manuscript source, generated
tables, figures, dependency lock, and matching PDF must be committed together,
and that immutable commit hash must be recorded in the submission. The
machine-readable benchmark manifest and audit report are stored under
\path{data/benchmarks/results_5000/}. No Git release tag is claimed.
...
Beyond the evaluated proxy, I implemented a typed, provenance-constrained evidence-graph prototype with a deterministic audit layer. Chapter~9 reports its thesis status: the corrected repository's 164-test backend suite passes and a small corruption-scenario probe demonstrates one concrete mechanism (source-independence clustering) by which it changes a decision relative to a simpler baseline. It remains intentionally separate from the proxy and therefore does not alter the 5{,}000-claim results.
...
through implementation and the corrected repository's 164-test suite, and evaluates the separate
...
Linux checkouts agree. Exact replay does not require web search. The exact
submission source and PDF must nevertheless be committed together and cited by
their immutable commit hash before submission.
...
The complete backend test suite was re-executed for this revision: \textbf{164 tests collected, 164 passed, 0 failed}. This includes the graph-specific cases and a regression test for the unified claim normalizer. Historical audit reports with 162 or 163 tests predate one or both of those additions.
...
Every path in this appendix is relative to the repository root. The revised
set is tracked on branch \path{thesis/reproducibility-corrections}, not
\texttt{main}. Historical audit commit \texttt{a6bbafc} predates this
correction; the exact submission commit must include this source and its
matching PDF. No release tag is asserted.

\begin{longtable}{p{5.1cm}p{7.7cm}}
\caption{Thesis-support artifacts on the correction branch.}\\
