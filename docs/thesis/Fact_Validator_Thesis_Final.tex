...
\vspace{0.4cm}
{\Large \textbf{A Deployment-Oriented and Auditable Prototype\\for Open-Domain Fact Verification}\par}
\vspace{0.6cm}
{\normalsize\itshape Master's Thesis\par}
{\normalsize\itshape Academic Year 2025--2026\par}
\vfill
...
\chapter*{Research and Reproducibility Statement}
\addcontentsline{toc}{chapter}{Research and Reproducibility Statement}
...
The research apparatus underlying this thesis is preserved in the following
immutable repository snapshot:
\begin{center}
\small\texttt{717b7f7f2fcaf700a0d76af9abb32dd5529ede52}
\end{center}
This commit is on branch \path{thesis/reproducibility-corrections}. The snapshot contains the
implementation, datasets, dependency lock, generated statistical artifacts,
and manuscript source used for the final verification pass. The
machine-readable benchmark manifest and audit report are stored under
\path{data/benchmarks/results_5000/}. The final manuscript source and matching
PDF are committed together in the subsequent provenance commit.
...
The document distinguishes executed measurements from recommendations; no
unexecuted experiment is presented as a result. The historical 5{,}000-claim
evaluation is reproducible as an artifact but exploratory rather than
confirmatory because development was informed by observations on that
benchmark and retrospective normalization found cross-split overlaps.

\paragraph{Scope of the thesis.} The repository contains two separate evaluation surfaces. The \system\ live application implements open-web retrieval, domain credibility scoring, semantic reranking, persistence, and optional Ollama debate. The 5{,}000-claim experiment evaluates the deterministic \proxy, which combines lexical classification, category-level priors, heuristic semantic signals, deterministic arbitration, and a quality filter. It does not execute SerpAPI, live domain scoring, SentenceTransformer reranking, or Ollama debate for every test claim. Chapter~9 documents a separate evidence-graph prototype that is not part of the 5{,}000-claim proxy evaluation.
...
The exploratory study uses a versioned 5{,}000-claim benchmark assembled from FEVER, LIAR, SciFact, and the PUBHEALTH \texttt{health\_fact} dataset stored under a legacy healthver compatibility name. The full \proxy\ reaches 50.82\% accuracy and 0.4384 macro-F1. A proxy version without deterministic debate reaches 51.34\% accuracy and 0.4427 macro-F1. A FEVER-aware proxy reaches 50.98\% accuracy and 0.4421 macro-F1. These descriptive findings are not measurements of the live application, are not statistically confirmatory, and are not evidence of state-of-the-art superiority.
...
Beyond the evaluated proxy, I implemented a typed, provenance-constrained evidence-graph prototype with a deterministic audit layer. Chapter~9 reports its thesis status: the audited repository's 164-test backend suite passes and a small corruption-scenario probe demonstrates one concrete mechanism (source-independence clustering) by which it changes a decision relative to a simpler baseline. It remains intentionally separate from the proxy and therefore does not alter the 5{,}000-claim results.
...
\chapter*{Contribution and Evidence Boundary}
\addcontentsline{toc}{chapter}{Contribution and Evidence Boundary}
The principal contribution is an engineering integration of open-web retrieval, explicit and inspectable source priors, evidence presentation, persistence, optional local-model arbitration, and reproducibility artifacts. The live application is implemented and software-tested. The deterministic proxy has an exploratory 5{,}000-claim evaluation whose small raw differences over majority and length baselines do not survive Holm correction. Chapter~9 documents a preliminary graph-auditor probe; eight synthetic cases do not establish a new empirical method.
...
\begin{enumerate}
  ...
  \item Formalize every metric used in the evaluation as an explicit mathematical definition, so that the empirical chapters can be read and checked independently of the code that produced them.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item I executed an exploratory 5{,}000-claim evaluation of the deterministic \proxy\ and compared it with multiple baselines and variants.
...
Dataset split and overlap audit & Completed & Strong normalization identified 19 train--validation, 34 train--test, and 5 validation--test exact overlaps; the 39 affected test claims are removed in a 4{,}961-claim sensitivity analysis. \\
5{,}000-claim proxy evaluation & Completed, exploratory & Exactly 5{,}000 aligned predictions per system are reported descriptively, alongside the exact-overlap sensitivity analysis. \\
...
Evidence-graph prototype & Implemented prototype & 164/164 backend tests pass on the audited repository; the prototype is reported separately from the proxy evaluation (Chapter~9). \\
...
The remainder of this thesis is organized as follows. Chapter~2 situates the work relative to prior literature. Chapter~3 gives the mathematical definitions of every metric used later. Chapter~4 describes the nine-stage architecture. Chapter~5 describes the dataset and experimental design. Chapter~6 reports the exploratory historical results. Chapter~7 discusses their meaning. Chapter~8 states limitations and threats to validity. Chapter~9 documents the preliminary evidence-graph prototype separately from the proxy. Chapter~10 gives a structured comparative and novelty analysis. Chapter~11 records corrections required for confirmatory evaluation. Chapter~12 sketches research directions, and Chapter~13 concludes. Appendices provide the audit-rule inventory, test coverage, corruption-probe materials, source-code listings, glossary, and mathematical notation.
...
These datasets were created for different purposes and use different annotation schemes. Combining them creates methodological challenges, but it also allows me to investigate whether the architecture behaves consistently across different domains. Table~\ref{tab:dataset-lit} summarizes the four source datasets on the dimensions that matter for this thesis: label granularity, evidence format, and domain.

Automated fact-checking is normally decomposed into claim detection,
evidence retrieval, evidence assessment, and verdict prediction
\citep{guo2022survey}. That decomposition is important here because software
tests of an end-to-end application, predictive evaluation of a proxy, and
human usefulness of presented evidence are different forms of evidence and
must not be substituted for one another.

\begin{table}[H]
\centering
\caption{Source datasets and their original task definitions.}
\label{tab:dataset-lit}
\scriptsize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.15cm}>{\raggedright\arraybackslash}p{3.15cm}>{\raggedright\arraybackslash}p{3.45cm}>{\raggedright\arraybackslash}X}
\toprule
...
PUBHEALTH (\texttt{health\_fact}) & Mixed public-health and general fact-checking claims & Numerical veracity labels & Article-level, heterogeneous sources \\
\bottomrule
\end{tabularx}
\end{table}
...
Truth-O-Meter combines web mining, natural-language inference, and
defeasible-logic argumentation for conflicting sources
\citep{galitsky2025truthometer}. Its dialectical reasoning is substantially
richer than the simple typed graph and deterministic rules in Chapter~9.
Truth-O-Meter's reported numbers are not directly comparable with this thesis:
the tasks, evidence conditions, label spaces, and evaluation procedures differ.
Its public repository and tutorial material are useful implementation context,
but they are not substitutes for the scholarly publication.

\section{Faithful Factual Correction}
Zero-shot Faithful Factual Error Correction studies the detection and correction of factual errors without task-specific retraining and evaluates correction faithfulness, including human judgments \citep{huang2023zeroshot}.

Its objective is primarily to correct text while preserving semantic faithfulness. My system primarily verifies and classifies; its correction functionality has not received a comparable benchmark or human evaluation.
...
\section{Leakage and Evaluation Independence}
Reproducible files are necessary but do not by themselves establish an
independent evaluation. Leakage can arise through preprocessing, duplicate
records, model selection, or repeated feedback from a nominal test set and can
produce overoptimistic conclusions \citep{kapoor2023leakage}. This distinction
motivates the retrospective overlap audit in Chapter~5 and the decision to
describe the 5{,}000-claim findings as exploratory.
...
\begin{table}[H]
\centering
\caption{Research positioning of \system\ relative to the principal literature threads above.}
...
\begin{tikzpicture}[
  node distance=.45cm,
  actor/.style={rectangle,draw,rounded corners,align=center,minimum width=2.15cm,minimum height=.95cm,fill=blue!6,font=\small},
  msg/.style={-{Latex[length=2mm]},thick},
  lifeline/.style={dashed,gray}]
\node[actor] (client) {Client\\(Next.js)};
\node[actor,right=of client] (api) {FastAPI\\\texttt{/analyze}};
\node[actor,right=of api] (retr) {Retrieval\\and Cache};
\node[actor,right=of retr] (cred) {Credibility\\and Rerank};
\node[actor,right=of cred] (verd) {Verdict\\and Debate};
\node[actor,right=of verd] (db) {SQLite\\Persistence};
\foreach \n in {client,api,retr,cred,verd,db} {
  \draw[lifeline] (\n.south) -- ++(0,-5.0);
}
\draw[msg] ([yshift=-0.6cm]client.south) -- node[above,font=\small]{claim or URL} ([yshift=-0.6cm]api.south);
\draw[msg] ([yshift=-1.25cm]api.south) -- node[above,font=\small]{query} ([yshift=-1.25cm]retr.south);
\draw[msg] ([yshift=-1.9cm]retr.south) -- node[above,font=\small]{evidence} ([yshift=-1.9cm]cred.south);
\draw[msg] ([yshift=-2.55cm]cred.south) -- node[above,font=\small]{scored evidence} ([yshift=-2.55cm]verd.south);
\draw[msg] ([yshift=-3.2cm]verd.south) -- node[above,font=\small]{run record} ([yshift=-3.2cm]db.south);
\draw[msg] ([yshift=-3.85cm]db.south) -- node[above,font=\small]{stored ID} ([yshift=-3.85cm]verd.south);
\draw[msg] ([yshift=-4.45cm]verd.south) -- node[above,font=\small]{verdict and confidence} ([yshift=-4.45cm]api.south);
\draw[msg] ([yshift=-4.9cm]api.south) -- node[above,font=\small]{response JSON} ([yshift=-4.9cm]client.south);
\end{tikzpicture}%
...
\section{Historical Exploratory Benchmark}
I created the evaluation family by combining claims from FEVER, LIAR, SciFact, and the PUBHEALTH \texttt{health\_fact} partition. The archived files contain 11{,}986 training claims, 2{,}997 validation claims, and 5{,}000 test claims. These counts describe records, not fully isolated semantic items.

\section{Dataset Composition}
\begin{table}[H]
\centering
\caption{Exploratory benchmark composition and label harmonization.}
...
PUBHEALTH partition & PUBHEALTH health\_fact & 6{,}000 & 1{,}490 & Mixture/unproven labels mapped to NEI by the historical script; stored under the legacy \texttt{healthver} compatibility filename. This is not interpreted as a clean medical-domain subset. \\
\bottomrule
\end{tabularx}
\end{table}
...
The original builder normalized claim text before splitting, whereas the
validator initially applied a weaker definition. I unified both paths using
NFKC normalization, case folding, punctuation removal, and whitespace
collapse. The resulting audit identified 19 train--validation, 34 train--test,
and 5 validation--test exact overlaps; 39 distinct test claims were affected.
I therefore report the original 5{,}000-claim result together with a
4{,}961-claim sensitivity analysis that excludes every known exact test
overlap. A separate audit also identified likely near-duplicates. Consequently,
the numerical findings remain exploratory even though the exact-overlap
sensitivity result is stable.

\section{Label Semantics and Sensitivity}
The three-way harmonization changes the target construct. FEVER's NEI means
that the supplied evidence is insufficient, whereas LIAR's ``half-true'' and
``barely-true'' and PUBHEALTH's ``mixture'' and ``unproven'' describe different
forms of partial truth or uncertainty. Mapping all four to NEI conflates
evidential insufficiency with mixed veracity. The present tables retain the
historical mapping solely to reproduce the archived experiment. A
confirmatory redesign should either exclude ambiguous source labels or add a
MIXED/PARTIAL class, then report both designs as a sensitivity analysis.
...
This chapter reports exploratory historical results of the deterministic \proxy. They do not characterize end-to-end execution of the live \system\ application and do not establish state-of-the-art performance. Because model development observed this benchmark and split overlap was found retrospectively, confidence intervals and paired tests are descriptive only; no confirmatory superiority claim is made.

\section{Final Model Performance}
\begin{table}[H]
\centering
\caption{Exploratory descriptive performance on the historical 5{,}000-claim benchmark. Every figure was re-verified against \texttt{ablation\_study\_report.json}.}
\label{tab:perf}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.1cm}>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}p{2.1cm}>{\raggedright\arraybackslash}p{1.7cm}>{\raggedright\arraybackslash}X}
\toprule
...
\section{Exact-Overlap Sensitivity Analysis}
Table~\ref{tab:decontaminated} compares the archived 5{,}000-claim evaluation
with the 4{,}961-claim subset obtained after excluding all 39 test claims that
have an exact normalized match in training or validation. The full proxy's
accuracy changes from 50.82\% to 50.78\%, a difference of $-0.044$ percentage
points. Its macro-F1 changes from 0.4384 to 0.4359. The no-debate variant
remains the highest-scoring proxy configuration. Thus, the known exact
overlaps do not materially explain the headline result.

\begin{table}[H]
\centering
\caption{Sensitivity of selected results to removal of 39 exact normalized test overlaps.}
\label{tab:decontaminated}
\small
\begin{tabularx}{\textwidth}{Xrrrrr}
\toprule
System & \multicolumn{2}{c}{Full ($n=5{,}000$)} & \multicolumn{2}{c}{Filtered ($n=4{,}961$)} & $\Delta$ Acc. \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
 & Accuracy & Macro-F1 & Accuracy & Macro-F1 & (pp) \\
\midrule
\proxy\ without debate & 51.34\% & 0.4427 & 51.30\% & 0.4402 & $-0.040$ \\
Full \proxy & 50.82\% & 0.4384 & 50.78\% & 0.4359 & $-0.044$ \\
Length heuristic & 49.42\% & 0.3483 & 49.61\% & 0.3486 & $+0.187$ \\
Majority baseline & 49.26\% & 0.2200 & 49.45\% & 0.2206 & $+0.186$ \\
\bottomrule
\end{tabularx}
\end{table}

This filtering is a robustness check rather than a new untouched test. It
does not remove all likely near-duplicates, and it does not reverse the fact
that proxy development was informed by observations on the benchmark. The
appropriate conclusion is therefore precise: exact normalized overlap has a
negligible measured effect, while broader evaluation-independence limitations
remain.
...
shortcuts, while also showing that the lexical and heuristic proxy is not merely
...
in the target domain rather than inferred from only the combined
score.
...
through implementation and the audited repository's 164-test suite, and evaluates the separate
...
The four datasets use different label systems. Mapping LIAR ``half-true'' and
``barely-true'' and PUBHEALTH ``mixture'' and ``unproven'' to NEI conflates
partial truth, mixed content, uncertainty, and insufficient evidence. The
partition labelled ``health'' also contains political and general
fact-checking claims and is not a clean medical-domain evaluation. Results for
that partition are descriptive of the source file, not of clinical validity.

\section{Internal Validity}
The 5{,}000-claim benchmark was observed during proxy development: the first
full run obtained 23.62\% accuracy, after which proxy classification and a
FEVER-specific adjustment were changed using feedback from the same benchmark.
The reported 50.82\% full-proxy result is therefore exploratory, not a blind
test result. Strong normalization also finds 39 test claims with exact
train/validation matches, plus additional likely near-duplicates. A future
confirmatory study must freeze implementation choices using training and
validation data only, then evaluate once on unused official or external data.
...
The historical proxy claims and predictions are retained and regenerated into
class, dataset, confusion-matrix, and paired-test reports by a versioned
script. Text-artifact hashes use canonical LF endings so that Windows and
Linux checkouts agree. Exact replay does not require web search. The audited
research snapshot is \texttt{717b7f7}; its full identifier is recorded in the
Research and Reproducibility Statement. The final manuscript source
and matching PDF are stored together in the subsequent provenance commit.
...
I want to state its status precisely. The module exists as working code on
branch \path{thesis/reproducibility-corrections}, separately from the proxy;
the backend suite passes 164 tests in the corrected environment; and the module
has been probed, at very small scale, with corruption scenarios constructed
against its own rule logic (Section~9.6). It has \emph{not} been integrated
into, or evaluated against, the 5{,}000-claim benchmark and should be read as
preliminary engineering work rather than an established empirical method.
...
The complete backend test suite was executed against the audited snapshot:
\textbf{164 tests collected, 164 passed, 0 failed}. This includes the
graph-specific cases and a regression test for the unified claim normalizer.
Historical audit reports with 162 or 163 tests predate one or both additions.

\subsection{Eight-Case Synthetic Seed}
A frozen eight-case seed, executed against the audited snapshot, reproduces
its recorded coverage, selective risk, and false-accept rate
(Section~\ref{sec:selective-defs}) exactly: 0.375, 0.6667, and 0.25
respectively. Table~\ref{tab:seed-detail-2} gives the per-case detail.
...
Empirical status & established benchmark & self-reported & self-reported & live: 164 tests on corrected branch; proxy: exploratory evaluation & \textbf{engineering-verified only} \\
\bottomrule
\end{tabularx}
\end{table}

\section{What Is Genuinely Novel}
The defensible contribution is systems integration, not a claim that any
individual formula, classifier, or graph rule is methodologically new:
\begin{enumerate}
  \item An engineering integration of open-web retrieval, an explicit formula-based domain-credibility prior, evidence presentation, persistence, optional local-model arbitration, and reproducibility artifacts.
  \item A transparent exploratory record with aligned claim identifiers, canonical line-ending-independent hashes, class and dataset metrics, exact paired tests, multiple-comparison correction, and deterministic regeneration.
  \item A preliminary graph-auditor prototype that demonstrates one designed abstention behavior on a small synthetic probe. This is future-work evidence, not a new validated empirical method.
\end{enumerate}
...
This thesis presents a complete engineering study with an exploratory
effectiveness evaluation. The following recommendations define the work
needed to progress from the present auditable prototype to a confirmatory,
...
\midrule
\endhead
Data verification & The split audit and 4{,}961-claim exact-overlap sensitivity analysis are documented. & Completed \\
Statistical analysis & Wilson intervals, paired tests, Holm correction, and sensitivity results are reported with exploratory interpretation. & Completed \\
...
Reproducibility & Claims, predictions, reports, manifests, code paths, run metadata, and the audited repository snapshot are documented. & Completed \\
...
Evidence-graph scoping & The prototype is a separate, tested contribution and is not merged into the 5{,}000-claim results. & Completed \\
...
The exploratory results support five bounded observations. First, the deterministic proxy
...
\chapter{Conclusion}
I designed and implemented \system\ as an auditable and deployment-oriented prototype for open-domain fact verification. The work includes a functional nine-stage architecture; a working full-stack implementation; explicit source-credibility reasoning; an exploratory deterministic proxy evaluation; per-class and per-dataset analysis; operational scenario projections; documented limitations; and a preliminary evidence-graph prototype.
...
The defensible contribution is a transparent, deployment-conscious
fact-verification prototype in which evidence, source trust, model behaviour,
and operational trade-offs can be inspected. Its value lies in engineering
integration and honest negative findings, not state-of-the-art predictive
accuracy or a validated new reasoning method.
...
The test evidence in this appendix is part of the audited repository snapshot
identified in the Research and Reproducibility Statement:
\begin{center}
\small\texttt{717b7f7f2fcaf700a0d76af9abb32dd5529ede52}
\end{center}
...
\bottomrule
\end{longtable}
Re-verified on 31 July 2026 by executing \texttt{python -m pytest services/api/tests -q} from the repository root: 164 tests collected, 164 passed, 0 failed, with six warning records in the current Windows environment. Historical 162- and 163-test records predate later regression tests. Warning totals and timing are environment-sensitive.
...
\begin{lstlisting}[caption={Unedited console output from the script above, executed against the audited repository snapshot.}]
...
\chapter{Reproducibility Artifacts and Verification Listings}
...
audited repository snapshot. The authoritative machine-readable outputs are stored
under \texttt{data/benchmarks/results\_5000/}.

\section{Repository Snapshot and Artifact Availability}
Every path in this appendix is relative to the repository root and is present
in audited snapshot \texttt{717b7f7} on branch
\path{thesis/reproducibility-corrections}; the full identifier is recorded in
the Research and Reproducibility Statement and Appendix~B. This snapshot supersedes historical
audit commit \texttt{a6bbafc}. The final manuscript source and matching PDF are
stored together in the subsequent provenance commit.

\begin{longtable}{p{5.1cm}p{7.7cm}}
\caption{Thesis-support artifacts in the audited repository snapshot.}\\
\toprule Artifact & Repository-relative path \\
\midrule
\endfirsthead
\toprule Artifact & Repository-relative path \\
\midrule
\endhead
Reproducibility audit & \path{data/benchmarks/results_5000/reproducibility_audit_report.json} \\
Statistics generator & \path{services/api/Scripts/run_thesis_statistics.py} \\
Artifact validator & \path{services/api/Scripts/validate_thesis_artifacts.py} \\
Manifest builder & \path{services/api/Scripts/build_thesis_run_manifest.py} \\
Exact-overlap sensitivity report & \path{data/benchmarks/results_5000/sensitivity_analysis_report.json} \\
Exact-overlap sensitivity table & \path{data/benchmarks/results_5000/sensitivity_analysis_metrics.csv} \\
Cache validator & \path{services/api/Scripts/validate_cache.py} \\
Operational projection & \path{services/api/Scripts/run_operational_projection.py} \\
Direct dependencies & \path{services/api/requirements.in} \\
Hash-pinned environment & \path{services/api/requirements.lock} \\
\bottomrule
\end{longtable}
...
The validator rejects missing manifest fields, claim-ID overlap,
prediction-count errors, claim-ID mismatch, hash mismatch, undeclared
normalized overlap, and generated reports that differ from the archived
statistics. Its \texttt{--require-disjoint} mode rejects any normalized
cross-split overlap for a future confirmatory set.
...
The committed audit records the branch and source commit, environment, dependency
...
When executed, the repository workflow installs the hash-pinned backend environment, executes
...
in \path{services/api/requirements.lock}.
...
Archived benchmark & A fixed, versioned historical split and prediction record. Archival stability does not imply that the set was untouched during development or free of leakage. \\
...
\bibitem{huang2023zeroshot} K.-H. Huang, H. P. Chan, and H. Ji, ``Zero-shot Faithful Factual Error Correction,'' in \emph{Proc. 61st Annual Meeting of the Association for Computational Linguistics (Volume 1: Long Papers)}, 2023, pp. 5660--5676, doi: 10.18653/v1/2023.acl-long.311.
\bibitem{galitsky2025truthometer} B. Galitsky, ``Truth-O-Meter: Collaborating with LLM in Fighting Its Hallucinations,'' in \emph{Interdependent Human-Machine Teams: The Path to Autonomy}, 2025, pp. 175--210, doi: 10.1016/B978-0-443-29246-0.00004-3.
...
\bibitem{guo2022survey} Z. Guo, M. Schlichtkrull, and A. Vlachos, ``A Survey on Automated Fact-Checking,'' \emph{Transactions of the Association for Computational Linguistics}, vol. 10, pp. 178--206, 2022, doi: 10.1162/tacl\_a\_00454.
\bibitem{kapoor2023leakage} S. Kapoor and A. Narayanan, ``Leakage and the Reproducibility Crisis in Machine-Learning-Based Science,'' \emph{Patterns}, vol. 4, no. 9, 2023.
\end{thebibliography}
