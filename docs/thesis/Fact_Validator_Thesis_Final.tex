...
The auditable repository snapshot supporting this thesis is the pushed branch
\path{thesis/reproducibility-corrections} at commit
\texttt{95c514c}. The audit itself was executed
from source commit \texttt{4612740}
and its report was added by the later commit. The exact environment,
dependency-lock hash, benchmark hashes, commands, and captured test output are
recorded in
\path{data/benchmarks/results_5000/reproducibility_audit_report.json}.
No Git release tag is claimed by this thesis. In particular,
\texttt{thesis-v1.0.0} was only a proposed identifier and was never created.
Readers must check out the correction branch above; the older
\texttt{origin/main} snapshot does not contain these thesis-support artifacts.
...
Beyond the evaluated proxy, I implemented a typed, provenance-constrained evidence-graph prototype with a deterministic audit layer. Chapter~9 reports its final thesis status: the audited correction-branch snapshot's 163-test backend suite passes and a small corruption-scenario probe demonstrates one concrete mechanism (source-independence clustering) by which it changes a decision relative to a simpler baseline. It remains intentionally separate from the proxy and therefore does not alter the 5{,}000-claim results.
...
Evidence-graph prototype & Completed prototype & 163/163 backend tests pass on the audited correction-branch snapshot; it is intentionally reported separately from the proxy evaluation (Chapter~9). \\
...
through implementation and the audited correction branch's 163-test suite, and evaluates the separate
...
I want to state its status precisely, because that precision is the point of including it at all. The module exists as working code on the audited correction branch, separately from the proxy; the complete backend suite at that snapshot passes 163 tests (re-executed and confirmed for this revision); and the module has been probed, at very small scale, with corruption scenarios constructed against its own rule logic (Section~9.6). It has \emph{not} been integrated into, or evaluated against, the 5{,}000-claim benchmark in Chapters~5--6 and should not be read as adding to, replacing, or explaining any number reported there.
...
The complete backend test suite was re-executed for this revision on the audited correction-branch snapshot: \textbf{163 tests collected, 163 passed, 0 failed}. The graph-specific cases are part of that branch-specific total. The older \texttt{main} snapshot contains one fewer integration test and therefore collects 162 tests.
...
Empirical status & established benchmark & self-reported & self-reported & live: 163 tests on audited branch; proxy: frozen evaluation & \textbf{engineering-verified only} \\
\bottomrule
\end{tabularx}
\end{table}
...
\bottomrule
\end{longtable}
Re-verified for this revision by executing \texttt{python -m pytest services/api/tests -q} from the repository root on the correction branch with Python 3.10.0: 163 tests collected, 163 passed, 0 failed, with three warning records. A fresh rerun on 29 July 2026 reproduced the same counts. The 162-test result obtained from \texttt{origin/main} is expected: commit \texttt{72ad99c} adds one recent-results integration test on the branch used by this thesis. Warning totals are environment-sensitive unless the hash-pinned dependency lock is used.
...
audited correction-branch snapshot. The authoritative machine-readable outputs are stored
under \texttt{data/benchmarks/results\_5000/}.

\section{Repository Snapshot and Artifact Availability}
Every path in this appendix is relative to the repository root. The complete
set is tracked on the pushed branch
\path{thesis/reproducibility-corrections}; it is not present in the older
\texttt{origin/main} snapshot. Commit
\texttt{95c514c} contains the support scripts,
dependency specifications, generated statistics, and audit report. The audit
report records source commit
\texttt{4612740}, because that was the
snapshot executed before the report itself was committed. No release tag is
asserted.

\begin{longtable}{p{5.1cm}p{7.7cm}}
\caption{Verified thesis-support artifacts on the correction branch.}\\
...
When executed, the correction-branch workflow installs the hash-pinned backend environment, executes
